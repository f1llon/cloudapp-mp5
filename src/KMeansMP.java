...
public final class KMeansMP {
    ...
    public static void main(String[] args) {
        ...
        final String results_path = args[1];
        final SparkConf sparkConf = new SparkConf().setAppName("KMeans MP");
        final JavaSparkContext sc = new JavaSparkContext(sparkConf);
        final JavaRDD<String> lines = sc.textFile(inputFile);
        final JavaRDD<Vector> points = lines.map(new ParsePoint());
        final JavaRDD<String> titles = lines.map(new ParseTitle());
        final KMeansModel model = KMeans.train(points.rdd(), 4, 100, 1, KMeans.RANDOM(), 0);
        final JavaPairRDD<Integer, Iterable<String>> results = titles.zip(points).mapToPair(new ClusterCars(model)).groupByKey();
